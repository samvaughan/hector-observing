...
n_changes = 0
for (index, row), radial_angle in zip(
    df.loc[np.isfinite(df["type"])].iterrows(), radial_angles
):
    angs = row.angs
    ...
    if np.abs(angs_degrees - radial_angle) < 10:
        print(
            f"Fixing Hexabundle {index} to be radial: {angs_degrees:.3f} --> {radial_angle:.3f}"
        )
    ...
    if np.abs(angs_degrees - (radial_angle + 180)) < 10:
        print(
            f"Fixing Hexabundle {index} to be radial: {angs_degrees:.3f} --> {radial_angle + 180:.3f}"
        )
