...
# hector_constants <<- read_yaml("/Users/z3536256/research/hector/Hector_config_test_August2022/HECTOR_CONSTANTS.yaml")
...
configure_HECTOR <- function(pos_master=pos_master){
  ...
  fieldflags=check_wedge_issue(pos=pos, angs=angs, gpos=guides$gpos[1:(ngprobes-1),], gangs=guides$gangs)
...
check_wedge_issue <- function(pos, angs, gpos, gangs){
  ...
  probes=defineprobe(x=pos[,'x'],y=pos[,'y'],angs=angs, interactive=T)
  gprobes=defineguideprobe(gxs=gpos[,'x'],gys=gpos[,'y'],gangs=gangs, interactive=T)
