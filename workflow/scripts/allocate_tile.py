...
output_folder_for_Allocation_folder = base_allocation_folder / 'tile_outputs' / f'{output_folder}'
...
HP = pipeline.HectorPipe(config_filename=config_filename, Profit_files_location=skyfiles_location, P_and_Q_offset_filename=P_and_Q_offset_filename)
...
for hexabundle in list(string.ascii_uppercase)[:21]:
    Path(base_allocation_folder / 'tile_outputs' / f'Fibre_coordData_hexabundle_{hexabundle}.txt').rename(output_folder_for_Allocation_folder / f'Fibre_coordData_hexabundle_{hexabundle}.txt')

# Move the HECTOROutput_Guides and HECTOROutput_Hexas files
Path(base_allocation_folder / 'tile_outputs' / f'HECTOROutput_Guides_{fname_stem}.txt').rename(output_folder_for_Allocation_folder / f'HECTOROutput_Guides_{fname_stem}.txt')
Path(base_allocation_folder / 'tile_outputs' / f'HECTOROutput_Hexas_{fname_stem}.txt').rename(output_folder_for_Allocation_folder / f'HECTOROutput_Hexas_{fname_stem}.txt')

# And now the slitInfo file
Path(base_allocation_folder / 'tile_outputs' / f'Fibre_slitInfo_{fname_stem}.csv').rename(output_folder_for_Allocation_folder / f'Fibre_slitInfo_{fname_stem}.csv')
...
for plot_fname in ['fibre_slitletAAOmega', 'fibre_slitletSpector', 'hexabundlePlot', 'robotPlot', 'skyfibre_slitletAAOmega', 'skyfibre_slitletSpector']:
    Path(base_plot_folder / f'{plot_fname}_{fname_stem}.pdf').rename(output_folder_for_Plots_folder / f'{plot_fname}_{fname_stem}.pdf')

print("...Done!")
# Now make the final files for Tony
final_tile_file = output_folder_for_Allocation_folder / f'HECTOROutput_Hexas_{fname_stem}.txt'
...
for prefix in ['Tile_FinalFormat', 'Robot_FinalFormat']:
    Path(base_final_files_folder / f'{prefix}_{fname_stem}.csv').rename(base_final_files_folder / f'{output_folder}' / f'{prefix}_{fname_stem}.csv')
