...
timeout<<-180 #Increased to account for longer conflict calculation time due to rounded heads so code still has enough time time try enough angles variations.
...
find_guide_conflicts <- function(probes, pos, angs, gprobes, gpos, gangs){
  ...
  print("Identifying clashes with target and/or standard probes")
  ...
  print('Identifying conflict with other guide probes.')
...
configure_HECTOR <- function(pos_master=pos_master){
  ...
  fieldflags=check_wedge_issue(pos=pos, angs=angs,gpos=guides$gpos,gangs=guides$gangs)
  print(paste('Wedge issue outcome: ',fieldflags))
  
  final_config=list(pos=pos, rads=rads, angs=angs, azAngs=azAngs, angs_azAng=angs_azAng, guidenum=guides$guidenum, gpos=guides$gpos, grads=grads, gangs=guides$gangs,gazAngs=gazAngs, gangs_gazAng=gangs_gazAng, swaps=swaps, flags=paste(fieldflags,stdflag,guides$guideflag,sep=''))
...
check_wedge_issue <- function(pos, angs, gpos, gangs){
  ...
  if (npinches>0) {
    ...
    for (pinch in c(1:npinches)){
      ...
      pinched=pinched[pinched%nin%pprobes]
      if (length(pinched)>0) {
    ...
    if (length(pinched>0)){fieldflags=paste(fieldflags,'check probe',as.character(pinched))}
    if (length(gpinched>0)){fieldflags=paste(fieldflags,'check guide',as.character(gpinched))}
  }
