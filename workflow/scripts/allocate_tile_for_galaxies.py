...
pipeline_config_dictionary = dict(output_folder=f"results/{smk.config['folder']}/TilingOutputs/{field}/", proximity=220, output_filename_stem='_')
...
base_allocation_folder = Path(f"{pipeline_config_dictionary['output_folder']}/Allocation/")
base_plot_folder = Path(f"{pipeline_config_dictionary['output_folder']}/Plots/")
base_final_files_folder = Path(f"{pipeline_config_dictionary['output_folder']}/FinalOutputs/")

output_folder_for_Allocation = base_allocation_folder / 'tile_outputs' / f'{tile_output_folder}'
...
try:
    ...
    print(f"Using a non-standard P and Q filename!! Currently {P_and_Q_offset_filename}")
except KeyError:
    P_and_Q_offset_filename = "Hexa_final_prism_gluing_PQ_table.xlsx"

HP = pipeline.HectorPipe(config_dictionary=pipeline_config_dictionary, Profit_files_location=skyfiles_location, P_and_Q_offset_filename=P_and_Q_offset_filename)

HP.allocate_hexabundles_for_single_tile(fileNameGuides=fname_Guides, fileNameHexa=fname_Hexas, robot_temperature=robot_temperature, obs_temperature=obs_temperature, plot=True)
...
for hexabundle in list(string.ascii_uppercase)[:21]:
    Path(base_allocation_folder / 'tile_outputs' / f'Fibre_coordData_hexabundle_{hexabundle}.txt').rename(output_folder_for_Allocation / f'Fibre_coordData_hexabundle_{hexabundle}.txt')

# Move the HECTOROutput_Guides and HECTOROutput_Hexas files
Path(base_allocation_folder / 'tile_outputs' / f'HECTOROutput_Guides_{fname_stem}.txt').rename(output_folder_for_Allocation / f'HECTOROutput_Guides_{fname_stem}.txt')
Path(base_allocation_folder / 'tile_outputs' / f'HECTOROutput_Hexas_{fname_stem}.txt').rename(output_folder_for_Allocation / f'HECTOROutput_Hexas_{fname_stem}.txt')

# And now the slitInfo file
Path(base_allocation_folder / 'tile_outputs' / f'Fibre_slitInfo_{fname_stem}.csv').rename(output_folder_for_Allocation / f'Fibre_slitInfo_{fname_stem}.csv')

# And now the Plots
for plot_fname in ['fibre_slitletAAOmega', 'fibre_slitletSpector', 'hexabundlePlot', 'robotPlot', 'skyfibre_slitletAAOmega', 'skyfibre_slitletSpector']:
    Path(base_plot_folder / f'{plot_fname}_{fname_stem}.pdf').rename(output_folder_for_Plots / f'{plot_fname}_{fname_stem}.pdf')
...
# Now move this file
for prefix, outfilename in zip(['Tile_FinalFormat', 'Robot_FinalFormat'], [smk.output.tile_file, smk.output.robot_file]):
